	...
	public class OreEaterV5 : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex)
		{
			...
			if (!modPlayer.OreEaterEnable)
			{
				...
				bool petProjectileNotSpawned = player.ownedProjectileCounts[ModContent.ProjectileType<Projectiles.OreEaterV5>()] <= 0;
					...
					for (int type = TextureAssets.Npc.Length - 1; type >= 0; type--)
					{
						...
						if (npcTexture == "AutoStacker/NPCs/OreEaterV5")
						{
							...
							Projectile.NewProjectile(player.GetProjectileSource_Buff(buffIndex), player.position.X + player.width / 2,
								player.position.Y + player.height / 2, 0f, 0f, ModContent.ProjectileType<Projectiles.OreEaterV5>(), 0, 0f, player.whoAmI);
							//modPlayer.pet.initListA();
							//modPlayer.pet.routeListX.Clear();
							//modPlayer.pet.routeListY.Clear();
							modPlayer.Pet = new PetV5();
							break;
						}
					}
			}
